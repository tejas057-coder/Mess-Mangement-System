...
function Navbar() {
          ...
          if (isInitialLoad.current) {
            ...
            if (localStorage.getItem("messmate_alerts_configured") === "granted") {
              ...
              newNotifs.forEach(n => {
                ...
                // Show floating popup toast in UI
                toast.info(
  ...
  useEffect(() => {
    ...
    // Initial fetch + poll every 5s for faster alert delivery
    fetchNotifications();
    ...
    <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      {alertPermissionState === "prompt" && (
        <div style={{
          ...
          boxShadow: `0 8px 24px ${accent.shadow}`, transition: "all 0.3s ease",
          zIndex: 1400,
        }}>
          ...
          <div style={{ display: "flex", gap: 10, flexShrink: 0 }}>
            <button 
              ...
              style={{
                border: "none", background: "#fff", color: accent.hex,
                padding: "6px 14px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 800,
                boxShadow: "0 4px 10px rgba(0,0,0,0.15)", transition: "all 0.2s"
              }}
            >
              Enable
            </button>
      ...
      <nav style={{
        ...
        <div style={{
          ...
          {/* Bell Button & Notifications Popover */}
          <div ref={notifRef} style={{ position: "relative" }}>
            ...
            {showNotifications && (
              <div style={{
                ...
                WebkitBackdropFilter: "var(--glass-blur)", border: "1px solid var(--border)",
                borderRadius: 20, boxShadow: "var(--shadow-lg)", zIndex: 1500,
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border-2)", paddingBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={markAllRead} title="Mark all read" style={{ border: "none", background: "transparent", color: "var(--accent)", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                      <FaCheckDouble />
                    </button>
                    <button onClick={clearAll} title="Clear all" style={{ border: "none", background: "transparent", color: "#dc2626", fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                      <FaTrashAlt />
                    </button>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {notifications.length === 0 ? (
                    <div style={{ padding: "30px 0", textAlign: "center", color: "var(--text-4)", fontSize: 13, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            border: "1px solid var(--border-2)", cursor: n.read_status ? "default" : "pointer",
                            transition: "background 0.2s", display: "flex", gap: 10, alignItems: "flex-start",
                            position: "relative",
                          }}
                        >
                          <span style={{ fontSize: 16, marginTop: 2 }}>{typeIcon}</span>
                          <div style={{ display: "flex", flexDirection: "column", gap: 3, flex: 1, minWidth: 0 }}>
                            <span style={{ fontSize: 12, fontWeight: n.read_status ? 600 : 800, color: "var(--text)", wordBreak: "break-word" }}>
                              {n.title}
                            </span>
                            <span style={{ fontSize: 11, color: "var(--text-3)", wordBreak: "break-word" }}>
                              {n.message}
                            </span>
                            <span style={{ fontSize: 9, color: "var(--text-4)", alignSelf: "flex-end", marginTop: 4 }}>
                              {new Date(n.created_at).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}
                            </span>
                          </div>
                          {!n.read_status && (
                            <div style={{ width: 6, height: 6, borderRadius: "50%", background: accent.hex, position: "absolute", top: 12, right: 12 }} />
                          )}
                        </div>
                      );
                    })
          ...
          <div style={{
            ...
            {!isMobile && (
              <div>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "var(--text)", whiteSpace: "nowrap" }}>
                  Mess Owner
                </p>
                <p style={{ margin: 0, fontSize: 11, color: "var(--text-4)" }}>Admin</p>
              </div>
            )}
          </div>
        </div>
